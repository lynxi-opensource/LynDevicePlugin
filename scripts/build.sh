...
for target in ${go_targets[@]}; do
    ...
    docker buildx build --platform linux/amd64,linux/arm64 -t ${image_prefix}lynxidocker/${target}:${version} ${out_dir} -f scripts/go.Dockerfile --build-arg BIN=${target} --push
done
...
docker buildx build --platform linux/amd64,linux/arm64 -t ${image_prefix}lynxidocker/lynsmi-service:${version} ${out_dir} -f scripts/rust.Dockerfile --build-arg BIN=lynsmi-service --push
...
docker buildx build --platform linux/amd64,linux/arm64 -t ${image_prefix}lynxidocker/lynxi-device-discovery:${version} ${out_dir} -f scripts/rust.Dockerfile --build-arg BIN=lynxi-device-discovery --push
